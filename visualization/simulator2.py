...
class CAR(object):
    def __init__(self, id_, from_, to_, speed_, planTime_):
        ...
        self.carColor = [int(value)
                         for value in np.random.random_integers(0, 255, [3])]
        # **** dynamic parameters ****#
...
class ROAD(object):
    def __init__(self, id_, length_, speed_, channel_, from_, to_, isDuplex_):
        ...
        # **** dynamic parameters ****#
        # absolute bucket
        self.forwardBucket = {i: [None for j in range(
            self.channel_)] for i in range(self.length_)}
        self.backwardBucket = {i: [None for j in range(self.channel_)] for i in
                               range(self.length_)} if self.isDuplex_ else None
        self.fx, self.fy, self.bx, self.by, self.forwardNum, self.backwardNum = [
            0], [0], [0], [0], [0], [0]
    ...
    def setBucket(self, crossId):
        ...
        if bucket == 'forward':
            self.provideBucket, self.px, self.py, self.provideDone, self.provideNum = \
                [self.forwardBucket, self.fx, self.fy,
                    self.forwardDone, self.forwardNum]
            if self.isDuplex_:
    ...
    def receiveCar(self, carId):
        ...
        for i in range(self.channel_):
            frontCarLoc = self.findCar(
                self.length_ - leftX - 1, self.length_ - 1, i, self.receiveBucket)
            # if no front car
            if frontCarLoc == -1:
...
class CROSS(object):
    def __init__(self, id_, north_, east_, south_, west_):
        ...
        self.provider = [[direction, self.roadIds[direction]]
        ...
        self.providerDirection = [self.provider[i][0]
                                  for i in range(self.provider.__len__())]
        self.provider = [self.provider[i][1]
                         for i in range(self.provider.__len__())]
        # **** dynamic parameters ****#
        self.readyCars = []
        self.leftCars = []
        ...
        for index in range(self.provider.__len__()):
            ...
            if nextCarId[index] != -1:
                ...
                if nextRoad[index] == -1:
                    ...
                    nextDirection.append(self.direction(
                        self.provider[index], nextRoad[index]))
            else:
                nextCar.append(-1)
                nextRoad.append(-1)
        ...
        for presentRoadIndex in range(self.provider.__len__()):
            while nextCar[presentRoadIndex] != -1:
                ...
                if nextRoad[presentRoadIndex] == -1:
                    ...
                    action = nextroad_.receiveCar(
                        nextCar[presentRoadIndex].__id__())
                    if action == 2:
                        # waiting conflict
                        break
                    self.update = True
                ...
                if nextCarId[presentRoadIndex] != -1:
                    ...
                    if nextRoad[presentRoadIndex] == -1:
                        ...
                        nextDirection[presentRoadIndex] = self.direction(
                            self.provider[presentRoadIndex], nextRoad[presentRoadIndex])
                else:
...
class visualization(object):
            ...
            for crossId in crossList:
                ...
                for roadId in validRoad:
                    ...
                    if not CROSSDICT[nextCrossId].__done__():
                        ...
                        nextX, nextY = self.crossRelativeLoc(
                            presentX, presntY, crossId, roadId)
                        # update location
                        CROSSDICT[nextCrossId].setLoc(nextX, nextY)
                        minX, minY, self.maxX, self.maxY =\
                            min(nextX, minX), min(nextY, minY), max(
                                nextX, self.maxX), max(nextY, self.maxY)
                        nextCrossList.append(nextCrossId)
            crossList = nextCrossList
        self.maxX, self.maxY = (
            self.maxX-minX+2)*self.crossDistance, (self.maxY-minY+2)*self.crossDistance
        for crossId in CROSSNAMESPACE:
            x, y = CROSSDICT[crossId].__loc__()
            CROSSDICT[crossId].setLoc(x-minX, y-minY)
            CROSSDICT[crossId].setMapLoc(
                (x - minX+1)*self.crossDistance, (y - minY+1)*self.crossDistance)
    ...
    def crossRelativeLoc(self, x, y, crossId, roadId):
        ...
        if roadDirection == 0:
            ...
            print("Cross(%d) don't interact with road(%d)" %
                  (self.id_, roadId))
    #
    # draw functions
    #
    ...
    def plotCross(self, crossId, img):
        ...
        cv.circle(img, (x, y), self.crossRadius,
                  color=self.crossColor, thickness=-1, lineType=-1)
        if crossId >= 10:
            xx, yy = int(x - 4*self.crossRadius /
                         5), int(y + self.crossRadius / 2)
        else:
            xx, yy = int(x - self.crossRadius/2), int(y + self.crossRadius / 2)
        cv.putText(img, str(crossId), (xx, yy),
    ...
    def plotRoad(self, roadId, img):
        ...
        # plot line
        cv.line(img, (fromX, fromY), (toX, toY),
                color=self.roadColor, thickness=2)
        # plot bucket
    ...
    def drawBucket(self, road, lane, img):
        ...
        fromX, fromY = CROSSDICT[road.__from__()].__mapLoc__()
        toX, toY = CROSSDICT[road.__to__()].__mapLoc__()
        XY, intervalXY, rectangleSize, channel2XY, length2XY = self.bucketDrawInitial(
            fromX, fromY, toX, toY, lane, length)
        for i in range(length):
            for j in range(channel):
                xRD, yRD = int(XY[0]+rectangleSize[0]
                               ), int(XY[1]+rectangleSize[1])
                if bucket[i][j] is None:
                    cv.rectangle(img, (int(XY[0]), int(
                        XY[1])), (xRD, yRD), (0, 0, 0), 1)
                else:
                    color = CARDICT[bucket[i][j]].__carColor__()
                    cv.rectangle(img, (int(XY[0]), int(
                        XY[1])), (xRD, yRD), color=color, thickness=-1)
    ...
    def bucketDrawInitial(self, fromX, fromY, toX, toY, lane, length):
        ...
        if direction == 'north':
            ...
            XY = [fromX - self.channelDistance - self.channelWidth,
                  toY - self.crossRadius * 2 - unitLength]
            intervalXY = -(self.channelDistance +
                           self.channelWidth), -unitLength
            rectangleSize = self.channelWidth, unitLength
            channel2XY, length2XY = 0, 1
        elif direction == 'east':
            XY = [toX - self.crossRadius * 2 -
                  unitLength, fromY + self.channelDistance]
            ...
            XY = [toX + self.crossRadius * 2, fromY -
                  self.channelDistance - self.channelWidth]
            intervalXY = unitLength, - \
                (self.channelDistance + self.channelWidth)
            rectangleSize = unitLength, self.channelWidth
            channel2XY, length2XY = 1, 0
...
def main():
    ...
    for line in carInfo:
        id_, from_, to_, speed_, planTime_ = line.replace(
            ' ', '').replace('\t', '')[1:-1].split(',')
        CARNAMESPACE.append(int(id_))
        CARDICT[int(id_)] = CAR(int(id_), int(from_),
                                int(to_), int(speed_), int(planTime_))
    # create road objects
    # line = (id,length,speed,channel,from,to,isDuplex)
    for line in roadInfo:
        id_, length_, speed_, channel_, from_, to_, isDuplex_ = line.replace(
            ' ', '').replace('\t', '')[1:-1].split(',')
    ...
    for line in crossInfo:
        id_, north_, east_, south_, west_ = line.replace(
            ' ', '').replace('\t', '')[1:-1].split(',')
        CROSSNAMESPACE.append(int(id_))
        CROSSDICT[int(id_)] = CROSS(int(id_), int(north_),
                                    int(east_), int(south_), int(west_))
    ...
    for carId in CARNAMESPACE:
        CROSSDICT[CARDICT[carId].__from__()].carportInitial(
            CARDICT[carId].__planTime__(), carId)
    # ****Initialization ****#
    CARNAMESPACE.sort()
    CROSSNAMESPACE.sort()
    # simulator
